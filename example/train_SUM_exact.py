...
from sum_exact import SUM_exact

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--method", default='SUM_exact', type=str, help="select optimizer")
    ...
    #base_optimizer = torch.optim.SUM_exact
    optimizer = SUM_exact(model.parameters(), t=args.t, lr=args.learning_rate, momentum=args.momentum, dampening=0, weight_decay=args.weight_decay,nesterov = False)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in dataset.train:
            ...
            # get the first term \grad E_{n}(x(tn)+\delta x1) and store it in self.state[p]["pre_grad"] 
